...
(function(root, factory) {
	...
	$.extend(Selectize.prototype, {
		...
		setup: function() {
			...
			$index_control    = $('#'+settings.warpperIndexId);
			
			var current_index = 9999;
			if($index_control && $index_control.length > 0){
				// update
				current_index = parseInt($index_control.html()) - 1;
				$index_control.html(current_index);
			}else{
				// create
				$('<div id="'+settings.warpperIndexId+'" style="display:none;">9999</div>').appendTo($('body'));
			}
			
			$wrapper.css({
				width: $input[0].style.width,
				"z-index":current_index
			});
			...
			$document.on('mousedown' + eventNS, function(e) {
				if (self.isFocused) {
                    ...
                    // prevent events on the dropdown 'ignoreMouseDownClass' called (modify by linpeng)
                    ...
                    // prevent events on the dropdown scrollbar case 'select_box' plugin
                    if(e.target.parentNode.parentNode === self.$dropdown[0]){
                    	return false;
                    }
		...
		onBlur: function(e) {
			...
			if (!self.ignoreBlur && document.activeElement === self.$dropdown_content[0]) {
				...
				// self.onFocus(e);
				// Q140117000066:IE bug,can not focus. by linpeng 2014-11-20
				self.isFocused = true;
				return;
			}
            // prevent IE closing the dropdown when the self.ignoreDropdownClass is clicked
            if (!self.ignoreBlur && ($(document.activeElement).hasClass(self.ignoreMouseDownClass) || $(document.activeElement).parents('.'+self.ignoreMouseDownClass).length > 0)) {
                self.ignoreBlur = true;
                self.onFocus(e);
                return;
            }
		...
		positionDropdown: function() {
			...
			//offset.top always equals 0 when $dropdown css position is 'relative',because IE6 and IE7 not work with 'absolute' (modify by linpeng 2014-8-25)
			//offset.top = 0;
	...
	Selectize.defaults = {
		...
		warpperIndexId: 'selectize-control-index-id',
    ...
    /**
     * dropdown header toolbar plugin,include selectall and clearall button
     */
	Selectize.define('toolbar_header', function(options) {
       if (this.settings.mode === 'single') return;
        ...
        self.clear = (function(){
            ...
            return function(){
                // Don't remove input in the case of pagination
		...
		self.setup = (function() {
           ...
           return function() {
               ...
               $('#'+self.$input.context.id+'_clear_all_btn').click(function(){
            	   // remove all item
                   if(self.items){
                       var new_items = self.items.slice(0);
                       $.each(new_items,function(i){
                           self.removeItem(new_items[i]);
                       });
                   }
...
//                          }
              ...
              self.$pagination.refreshButtonState = function(){
                  ...
                  // disabled next and last button when max_page equals 0 Add by linpeng 2014-8-26
                  if(self.$pagination.max_page == 0){
                	  $('#'+self.$input.context.id+'_navi_next').attr("disabled","disabled");
                      $('#'+self.$input.context.id+'_navi_last').attr("disabled","disabled");
                  }
              };
    ...
    Selectize.define('select_box', function(options) {
        ...
        options = $.extend({
            ...
            headerHtml :function(data){
                ...
                if(data.columns){
                    ...
                    result = '<div class="' + data.headerClass + '">' +
        ...
        self.settings.onItemAdd = (function(){
        	var original = self.settings.onItemAdd;
        	return function(){
        		if(original)
        			original.apply(self,arguments);
                ...
                var $info = $(self.render('info',self.options[arguments[0]]));
        ...
        self.settings.onItemRemove = (function(){
        	var original = self.settings.onItemRemove;
        	return function(){
        		if(original)
        			original.apply(self,arguments);
        ...
        self.settings.render = {
            ...
            option : function(item,escape){
                ...
                if(options.columns){
                	...
                	rowHtml = '<li class="'+options.labelClass+'" style="width:100%;">' +item[self.settings.labelField] + '</li>';
                }
        ...
        self.setup = (function() {
            ...
            return function() {
                original.apply(self, arguments);
                //self.$selectbox.content = $(options.html(options));
                //self.$dropdown.append(self.$selectbox.content);
                ...
                if(self.$toolbar_header){ // generate by plugin[toolbar_header]
                    self.$toolbar_header.append(self.$selectbox_header);
                }else{
                    self.$dropdown.prepend(self.$selectbox_header);
                }
    ...
    /**
     * clear button for single mode
     */
    Selectize.define('single_clear_button', function(options) {
            options = $.extend({
                label     : '&times;',
                title     : 'Remove',
            ...
            this.setup = (function() {
                ...
                return function() {
                    ...
                    // add event listener
                    this.$control.on('mousedown', '.' + options.className, function(e) {
                        if (self.isLocked) return;
                        var $value = $(e.currentTarget).parent().children('.item').data('value');
                        self.removeItem($value);
                    });
